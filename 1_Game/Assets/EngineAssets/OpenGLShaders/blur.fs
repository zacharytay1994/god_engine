...
float intensity = 1.5 ; // Additional blur added to whole scene
...
void main( )
{
    ...
    if ( uHorizontal )
    {
        for ( int i = 1; i < 5; ++i )
        {
            result += texture( uImage , vTexCoord + vec2( tex_offset.x * i * intensity , 0.0 ) ).rgb * uWeight[i];
            result += texture( uImage , vTexCoord - vec2( tex_offset.x * i * intensity , 0.0 ) ).rgb * uWeight[i];
        }
    }
    else
    {
        for ( int i = 1; i < 5; ++i )
        {
            result += texture( uImage , vTexCoord + vec2( 0.0 , tex_offset.y * i * intensity ) ).rgb * uWeight[i];
            result += texture( uImage , vTexCoord - vec2( 0.0 , tex_offset.y * i * intensity ) ).rgb * uWeight[i];
        }
    }
